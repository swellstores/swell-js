import { m as methods } from './cart.dd00d6d6.mjs';
import { m as methods$1 } from './settings.6c0d2563.mjs';
import { n as isFunction, o as isObject, C as loadScript, a as cloneDeep, f as toSnake, z as vaultRequest, p as toLower, r as isEmpty, x as reduce, D as isLiveMode, w as map, A as getLocationParams, B as removeUrlParams, t as toCamel } from './index.b29eadc6.mjs';
import { g as get, p as pick, t as toNumber } from './_baseClone.c22e5907.mjs';
import 'qs';
import 'deepmerge';
import 'fast-case';
...
class PaymentController {
  ...
  async _performPaymentAction(paymentInstances, action, ...args) {
    const paymentActions = paymentInstances.reduce((acc, paymentInstance) => {
      const paymentAction = paymentInstance[action];

      if (paymentAction) {
        acc.set(paymentInstance, paymentAction.call(paymentInstance, ...args));
      }

      return acc;
    }, new Map());

    const paymentActionResults = await Promise.allSettled(
      paymentActions.values(),
    );

    console.log(paymentActionResults);
    debugger;
