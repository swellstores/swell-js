(function (global, factory) {
	...
	function getProductOptionIndex(product, filter = void 0) {
	  ...
	  return reduce(
	    ...
	    (acc, op) => {
	      const values = reduce(
	        op.values,
	        (acc2, val) => __spreadProps$8(__spreadValues$f({}, acc2), {
	          [val.id]: __spreadProps$8(__spreadValues$f({}, val), { id: val.id }),
	          [val.name]: __spreadProps$8(__spreadValues$f({}, val), { id: val.id })
	        }),
	        {}
	      );
	      return __spreadProps$8(__spreadValues$f({}, acc), {
	        [op.id]: __spreadProps$8(__spreadValues$f({}, op), { values }),
	        [op.name]: __spreadProps$8(__spreadValues$f({}, op), { values })
	      });
	    },
	    {}
	  );
	...
	function calculateVariation(input, options, purchaseOption) {
	  ...
	  const variation = __spreadProps$8(__spreadValues$f({}, product), {
	...
	function findPurchaseOption(product, purchaseOption) {
	  ...
	  if (option) {
	    ...
	    return __spreadProps$8(__spreadValues$f({}, option), {
	...
	async function getFilterableAttributeFilters(request, products, options) {
	  ...
	  return getFilters(products, __spreadProps$8(__spreadValues$f({}, options), { filterableAttributes }));
	}
	      ...
	      if (product.attributes) {
	        for (let id in product.attributes) {
	          ...
	          if (attr) {
	            ...
	            attributes.push(__spreadProps$8(__spreadValues$f({}, product.attributes[id]), {
	              value: void 0,
	  ...
	  return {
	    ...
	    getItemData(item, data = {}) {
	      ...
	      if (typeof item === "string") {
	        result = __spreadProps$7(__spreadValues$e({}, data || {}), {
	          product_id: item
	        });
	      }
	    ...
	    update(input) {
	      ...
	      if (data.items && data.items.map) {
	        data = __spreadProps$7(__spreadValues$e({}, data), {
	    ...
	    decodeLocale(values) {
	      ...
	      if (configs) {
	        configs = configs.reduce(
	          (acc, config) => __spreadProps$6(__spreadValues$c({}, acc), {
	            [config.code]: config
	          }),
	          {}
	        );
	...
	class Payment {
	  ...
	  async getCart() {
	    ...
	    if (!cart.settings) {
	      ...
	      cart.settings = __spreadValues$b({}, settings.store);
	    }
	    return toSnake(cart);
	  }
	  ...
	  async _populateScriptWithCartParams(script) {
	    ...
	    const cart = await this.getCart();
	    script.params = __spreadValues$b(__spreadValues$b({}, script.params), pick(cart, cartParams));
