(function (global, factory) {
	...
	async function shouldUsePayPalEmail(guest, request, options2) {
	  if (!guest)
	    return false;
	  const updatedCart = await methods$9(request, options2).get();
	  ...
	  paypal.Buttons(
	    {
	      ...
	      onApprove: (data, actions) => actions.order.get().then(async (order) => {
	        ...
	        return methods$9(request).update(__spreadProps(__spreadValues$1({}, usePayPalEmail && {
	          account: {
	  ...
	  braintree.client.create({
	    ...
	    return paypal.Buttons({
	      ...
	      onApprove: (data, actions) => paypalCheckoutInstance.tokenizePayment(data).then(
	        ({ nonce }) => methods$9(request, options$1).update({
	  ...
	  if (params.card && payMethods.card) {
	    if (payMethods.card.gateway === "stripe" && CARD_ELEMENTS.stripe && API.stripe) {
	      ...
	      if (!paymentMethod) {
	        ...
	        return methods$9(request, options$1).update({
	          billing: {
	            method: "card",
	            card: paymentMethod
	          }
	      ...
	      if (intent && ["requires_capture", "requires_confirmation"].includes(intent.status)) {
	        ...
	        return await methods$9(request, options$1).update({
	          billing: {
	            method: "card",
	            card: paymentMethod,
	            intent: {
	    ...
	    if (payMethods.klarna.gateway === "klarna") {
	      ...
	      return error ? onError(error) : methods$9(request, options$1).update({
	        billing: {
	          method: "klarna"
	        }
