(function (global, factory) {
	...
	const cacheApi = {
	  ...
	  set(details) {
	    ...
	    if (path || value instanceof Array) {
	      let upData = __spreadValues$f({}, data || {});
	...
	function getProductOptionIndex(product, filter = void 0) {
	  ...
	  return reduce(
	    ...
	    (acc, op) => {
	      const values = reduce(
	        op.values,
	        (acc2, val) => __spreadProps$8(__spreadValues$e({}, acc2), {
	          [val.id]: __spreadProps$8(__spreadValues$e({}, val), { id: val.id }),
	          [val.name]: __spreadProps$8(__spreadValues$e({}, val), { id: val.id })
	        }),
	        {}
	      );
	      return __spreadProps$8(__spreadValues$e({}, acc), {
	        [op.id]: __spreadProps$8(__spreadValues$e({}, op), { values }),
	        [op.name]: __spreadProps$8(__spreadValues$e({}, op), { values })
	      });
	    },
	    {}
	  );
	...
	function calculateVariation(input, options, purchaseOption) {
	  ...
	  const variation = __spreadProps$8(__spreadValues$e({}, product), {
	...
	function findPurchaseOption(product, purchaseOption) {
	  ...
	  if (option) {
	    ...
	    return __spreadProps$8(__spreadValues$e({}, option), {
	...
	async function getFilterableAttributeFilters(request, products, options) {
	  ...
	  return getFilters(products, __spreadProps$8(__spreadValues$e({}, options), { filterableAttributes }));
	}
	      ...
	      if (product.attributes) {
	        for (let id in product.attributes) {
	          ...
	          if (attr) {
	            ...
	            attributes.push(__spreadProps$8(__spreadValues$e({}, product.attributes[id]), {
	              value: void 0,
	  ...
	  return {
	    ...
	    getItemData(item, data = {}) {
	      ...
	      if (typeof item === "string") {
	        result = __spreadProps$7(__spreadValues$d({}, data || {}), {
	          product_id: item
	        });
	      }
	    ...
	    update(input) {
	      ...
	      if (data.items && data.items.map) {
	        data = __spreadProps$7(__spreadValues$d({}, data), {
	  ...
	  return {
	    get: (type, id, query) => {
	      return cacheApi.getFetch(
	        `content_${type}`,
	        id,
	        () => request("get", `/content/${type}`, id, __spreadValues$c({
	    ...
	    decodeLocale(values) {
	      ...
	      if (configs) {
	        configs = configs.reduce(
	          (acc, config) => __spreadProps$6(__spreadValues$b({}, acc), {
	            [config.code]: config
	          }),
	          {}
	        );
	...
	function decodeLocaleValue(locale, values, key, configs, opt) {
	  ...
	  if (returnLocaleConfig) {
	    ...
	    while (fallbackKey) {
	      ...
	      fallbackValues = __spreadValues$b(__spreadValues$b({}, values[key][fallbackKey] || {}), fallbackValues);
	  ...
	  let localeValues = __spreadValues$b(__spreadValues$b({}, fallbackValues), values[key][returnLocaleKey] || {});
	...
	function getBillingDetails(cart) {
	  const details = __spreadValues$a({}, mapValues(billingFieldsMap, cart.billing));
	...
	function setKlarnaBillingShipping(source, data) {
	  ...
	  source.klarna = __spreadValues$a(__spreadValues$a({}, source.klarna), fillValues(shippingNameFieldsMap, data.shipping));
	  ...
	  if (shipping || shippingAddress) {
	    source.source_order.shipping = __spreadValues$a(__spreadValues$a({}, shipping ? shipping : {}), shippingAddress ? { address: shippingAddress } : {});
	  }
	  source.klarna = __spreadValues$a(__spreadValues$a({}, source.klarna), fillValues(
	  ...
	  if (billing || billingAddress) {
	    source.owner = __spreadValues$a(__spreadValues$a({}, billing ? billing : {}), billingAddress ? { address: billingAddress } : {});
	  }
	}
	function setBancontactOwner(source, data) {
	  ...
	  const billingData = __spreadValues$a(__spreadValues$a(__spreadValues$a(__spreadValues$a({}, account.shipping), account.billing), shipping), billing);
	  const billingAddress = fillValues(addressFieldsMap$1, billingData);
	  source.owner = __spreadValues$a(__spreadValues$a({
	    email: account.email,
	...
	async function createIDealPaymentMethod(stripe, element, cart) {
	  const billingDetails = getBillingDetails(cart);
	  return await stripe.createPaymentMethod(__spreadValues$a({
	    type: "ideal",
	    ideal: element
	...
	class StripeIDealPayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$5(__spreadValues$8({}, methods.ideal), {
	...
	class StripeBancontactPayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$4(__spreadValues$7({}, methods.bancontact), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	...
	class StripeKlarnaPayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$3(__spreadValues$6({}, methods.klarna), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	  ...
	  async tokenize() {
	    ...
	    const { source, error: sourceError } = await createKlarnaSource(
	      this.stripe,
	      __spreadProps$3(__spreadValues$6({}, cart), {
	...
	class StripeGooglePayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$2(__spreadValues$5({}, methods.google), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	...
	class StripeApplePayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$1(__spreadValues$4({}, methods.apple), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	  ...
	  _createPaymentRequest(cart) {
	    ...
	    const paymentRequest = this.stripe.paymentRequest(__spreadValues$4({
	  ...
	  async _onShippingAddressChange(event) {
	    ...
	    const cart = await this.updateCart({
	      shipping: __spreadProps$1(__spreadValues$4({}, shipping), { service: null }),
	      shipment_rating: null
	    });
	    if (cart) {
	      updateWith(__spreadValues$4({ status: "success" }, this._getPaymentRequestData(cart)));
	    } else {
	  ...
	  async _onPaymentMethod(event) {
	    ...
	    const { require: { shipping: requireShipping } = {} } = this.params;
	    await this.updateCart(__spreadProps$1(__spreadValues$4({
	      account: {
	        name: payerName,
	        email: payerEmail
	      }
	    }, requireShipping && {
	      shipping: __spreadProps$1(__spreadValues$4({}, this._mapShippingAddress(shippingAddress)), {
	        service: shippingOption.id
	      })
	    }), {
	      billing: __spreadProps$1(__spreadValues$4({}, this._mapBillingAddress(billing_details)), {
	        method: "card",
	        card: {
	          gateway: "stripe",
	          token: paymentMethod,
	...
	const mapAddressFields = (cart, addressField) => __spreadProps(__spreadValues$1({}, mapFields(addressFieldsMap, cart[addressField])), {
