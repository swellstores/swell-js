(function (global, factory) {
	...
	class Payment {
	  ...
	  async _populateScriptWithCartParams(script) {
	    ...
	    const cart = await this.getCart();
	    script.params = __spreadValues$g(__spreadValues$g({}, script.params), pick(cart, cartParams));
	...
	class StripeGooglePayment extends Payment {
	  ...
	  async _submitPayment(paymentData) {
	    ...
	    const { card } = token;
	    await this.updateCart(__spreadValues$a({
	      account: {
	        email
	      },
	      billing: __spreadValues$a({
	        method: "card",
	        card: {
	          token: token.id,
	          brand: card.brand,
	...
	class StripeApplePayment extends Payment {
	  ...
	  async _onShippingAddressChange(event) {
	    ...
	    if (cart) {
	      updateWith(__spreadValues$9({ status: "success" }, this._getPaymentRequestData(cart)));
	    } else {
